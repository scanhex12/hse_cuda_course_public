...
TrtUniquePtr<IHostMemory> buildSerializedEngine(const std::string& onnx_file, IBuilder* builder) {
  (void)onnx_file;
  (void)builder;
  throw std::runtime_error("TODO: buildSerializedEngine (см. README)");
}

void preprocess(const std::vector<uint8_t>& input_image, std::vector<float>& output, int width, int height) {
  (void)input_image;
  (void)output;
  (void)width;
  (void)height;
  throw std::runtime_error("TODO: preprocess — RGB HWC → NCHW float, ImageNet mean/std");
}
...
std::vector<std::pair<int, float>> inferImage(InferenceEngine* engine,
                                               ThreadContext* ctx,
...
class InferenceServiceImpl final : public trt::inference::v1::InferenceService::Service {
public:
  InferenceServiceImpl(InferenceEngine* engine, ThreadContext* ctx, std::mutex* infer_mutex)
  ...
  grpc::Status Classify(grpc::ServerContext* /*context*/,
    ...
    if (width <= 0 || height <= 0 || width > 10000 || height > 10000) {
      return grpc::Status(grpc::StatusCode::INVALID_ARGUMENT, "invalid width/height");
    }
    ...
    if (static_cast<size_t>(request->rgb_image().size()) != expected) {
      return grpc::Status(grpc::StatusCode::INVALID_ARGUMENT, "rgb_image size mismatch");
    }

    std::vector<uint8_t> image(request->rgb_image().begin(), request->rgb_image().end());
