...
void testCase(const std::vector<std::vector<int>> & src, const std::vector<std::vector<int>> & expected_result)
{
        ...
        for (size_t i = 0; i < expected_result.size(); ++i)
    {
        ...
        {
            REQUIRE(expected_result[i][j] == result.data[i * expected_result[i].size() + j]);
        }
    }
